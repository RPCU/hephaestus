...
let
  ...
  installKubevip = pkgs.writeShellScriptBin "installKubevip" ''
    ...
    kubevip manifest pod --interface ${privateInterface}.${privateVlanid} --address ${apiserverVip} --controlplane --services --arp --leaderElection --k8sConfigPath=/etc/kubernetes/super-admin.conf | tee /etc/kubernetes/manifests/kube-vip.yaml
  '';
in
{
  environment = {
